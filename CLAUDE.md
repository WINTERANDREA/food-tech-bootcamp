...
- **Profiling 2° giro 2026-04-24/25 — congelato**. Slot 1 (64 prospect): 22 OK / 42 FAIL per rate limit five_hour. Slot 2 (20 dei falliti): 3 OK / 17 FAIL per output JSON malformato + Playwright costoso ($1+ a prospect). **+3 nuovi qualificati totali** (tutti dallo slot 1): La Manierina Milano (8), Cesarino (7), el gian enoteca (7). Pool finale: 247 prospect, 153 scorati, **16 qualificati ≥7** (8: 2 / 7: 14), 94 senza fit_score. Tutti e 16 i qualificati hanno già menu_structured + briefing_cards. Diminishing returns confermati — pool 16 sufficiente per pilot 10. Profiling on hold.
...
- **CRM multi-zona committato** (commit `680deb3` branch `feat/ai-sales-agent`): feature "progetti" live — pagine `ProjectsList/ProjectDetail/ProjectNew`, `FitScoreChip`, sidebar rifatta (`SidebarFilters/Groups/Projects`), `TopNav`, SQL migrations già eseguite su Supabase. Push remote ancora pending (sandbox block — Andrea fa `git push` manuale).
- **Prossimo milestone**: (1) push remote (FTB + CRM + menu-extraction) → (2) deploy CRM su Vercel con feature projects → (3) deploy Voice Twin v0.3 in `generate-draft` edge function con `NO-32193` → (4) selezione 10 prospect pilota tra i 16 qualificati → (5) generazione draft Claudia → (6) review in `/review` → (7) prima cold email via Resend. Opzionale: rilanciare profiling sui 42 falliti + 55 ancora senza score quando il five_hour rate limit consente.
