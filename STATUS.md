...
| 2026-04-24 | **Commit sequenziali + profiling 2° giro.** (1) **Commit CRM `680deb3`** (branch `feat/ai-sales-agent`): feature "progetti multi-zona" pushata localmente — nuove pagine `ProjectsList/ProjectDetail/ProjectNew`, `FitScoreChip`, sidebar rifatta, `TopNav`, 3 script SQL già eseguiti su Supabase per cortesia di Andrea. (2) **Commit FTB `623eaf4`** + **menu-extraction `79a6b8a`** (flag `--project-id N` su tutti e 4 i batch runner per filtrare input contro `company_projects`). (3) Push remoto bloccato dal sandbox — Andrea fa `git push` a mano sui 3 repo. (4) Lanciato **2° giro di profiling sui 64 prospect pending** (filtro `link NOT NULL` + fit_score `NULL`). Risultato: **22 OK / 42 FAIL** — il batch ha saturato il rate limit five_hour della subscription Claude (`RateLimitEvent status='rejected'` al 22° prospect). Reset avvenuto alle 21:20. Nuovi qualificati ≥ 7: **La Manierina Milano (fit=8)**, **Cesarino (fit=7)**, **el gian enoteca (fit=7)**. Pool totale ora: 247 prospect, 150 scorati, **16 qualificati ≥ 7** (era 13), 97 ancora senza fit_score (42 falliti + 55 mai processati). Tutti e 16 i qualificati hanno già `menu_structured` + `briefing_cards` (i 3 nuovi avevano menu estratto pre-reorder). | +3 qualificati già pronti per email draft (menu+briefing presenti); rate-limit noto come freno operativo per batch >22 prospect in 5h |
| 2026-04-25 | **Profiling slot 2 — diminishing returns.** Rilanciato batch su 20 dei 42 falliti del giro precedente (reset rate-limit avvenuto). Risultato: **3 OK / 17 FAIL** (Boscovich Bistrot fit=5, Cesarino sede 2 fit=5, Vecchio Mulino non mirrored). Nessun nuovo qualificato ≥7 — il pool resta a **16**. Costo virtuale alto: il primo prospect (Vecchio Mulino) ha consumato ~$1 di compute Playwright facendo 7+ pagine prima di salvare il profile, e molti fallimenti non sono per rate-limit ma per output JSON malformato che il batch_profiling.py non riesce a parsare per il mirror Supabase. **Lezione**: i prospect rimasti hanno siti SPA/JS-heavy che richiedono Playwright fallback costoso, e i pochi che passano scorano sotto soglia. ROI marginale del profiling è basso. **Decisione operativa**: chiudere il profiling per ora, procedere al deploy Voice Twin v0.3 sui 16 qualificati esistenti (sufficienti per il pilot di 10 prospect). Pool finale: 247 totali, 153 scorati, 94 unscored, 16 qualificati. | Pipeline profiling congelata; focus sposta su deploy Voice Twin v0.3 + selezione 10 prospect pilota tra i 16 qualificati |
